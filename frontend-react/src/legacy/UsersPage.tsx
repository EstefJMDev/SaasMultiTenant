      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const canViewEmployee =
            !user.is_super_admin && employeeByUserId.has(user.id);
          const canDelete = !user.is_super_admin;
          if (!canEdit && !canViewEmployee && !canDelete) {
            return null;
          }
          return (
            <Menu placement="bottom-end">
              <MenuButton
                as={IconButton}
                aria-label={t("users.table.actions", "Acciones")}
              ...
              <MenuList>
                {canEdit && (
                  <MenuItem onClick={() => openEditUser(user)}>
                    {t("users.table.edit")}
                  </MenuItem>
                )}
                {canViewEmployee && (
                  <MenuItem onClick={() => handleViewEmployee(user)}>
                    Ver empleado
                  </MenuItem>
                )}
                {canDelete && (
