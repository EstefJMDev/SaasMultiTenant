...
export const TenantSettingsPage: React.FC = () => {
  ...
  const createTenantMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const detail = extractApiError(error, t("tenantSettings.messages.createErrorFallback"));
      toast({
  ...
  const updateTenantMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const detail = extractApiError(error, t("tenantSettings.messages.updateErrorFallback"));
      toast({
  ...
  // Desactiva un tenant existente.
  const handleDeactivateTenant = (tenantId: number) => {
    apiClient
      ...
      .catch((error: unknown) => {
        const detail = extractApiError(error, t("tenantSettings.messages.deleteErrorFallback"));
        toast({
        ...
        <>
          <Box mt={2}>
            <Stack direction={{ base: "column", md: "row" }} justify="space-between" align="center" mb={4}>
              ...
              <Button colorScheme="brand" size="sm" onClick={() => setCreateModalOpen(true)}>
