...
LEGACY_TIME_SUNSET = "Wed, 31 Dec 2026 23:59:59 GMT"
...
def mark_legacy_time_alias(request: Request, response: Response) -> None:
    logger.warning("Legacy time route used: %s", request.url.path)
    tenant_id = request.headers.get("X-Tenant-Id")
    resolved_tenant = int(tenant_id) if tenant_id and tenant_id.isdigit() else None
    mark_legacy_route("time_legacy", None, resolved_tenant)
    response.headers["Deprecation"] = "true"
    response.headers["Sunset"] = LEGACY_TIME_SUNSET
